...
/// Mutates the `out` argument to refer to a `value` containing a newly created JavaScript Object.
pub unsafe extern "C" fn new(out: &mut Local, env: Env) {
    napi::create_object(env, out as *mut _);
}

/// Mutates the `out` argument to refer to a `value` containing the own property names of the
...
/// Sets the key value of a `value` at the `index` provided. Returns `true` if the set
/// succeeded.
///
...
pub unsafe extern "C" fn get_string(env: Env, out: &mut Local, object: Local, key: *const u8, len: i32) -> bool {
    ...
    // Not using get_named_property() because the `key` may not be null terminated.
...
/// Sets the key value of a `value` at a named key. Returns `true` if the set succeeded.
///
/// The `out` parameter and the return value contain the same information for historical reasons,
/// see [discussion].
///
...
/// Sets the property value of an `value` object, named by another `value` `key`. Returns `true` if the set succeeded.
///
/// The `out` parameter and the return value contain the same information for historical reasons,
/// see [discussion].
///
